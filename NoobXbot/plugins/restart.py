...
from NoobXbot import SUDOERS, app
from NoobXbot.NoobXUtilities.database.functions import start_restart_stage
...
async def theme_func(_, message):
    ...
    for x in served_chats:
        try:
            await app.send_message(
                x,
                "NoobXbot has just restarted herself. Sorry for the issues.\n\nStart playing after 10-15 seconds again.",
            )
        except Exception:
            pass
    served_chatss = []
    ...
    x = await message.reply_text(f"__Restarting NoobX!__")
