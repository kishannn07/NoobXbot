...
async def vplay(c: Client, message: Message):
    ...
    if a.status != "administrator":
        await message.reply_text(
            f"""
💡 Make Me Admin With These Powers :
...
""",
    ...
    try:
        ...
        if b.status == "kicked":
            await message.reply_text(
                f"@{ubot.username} **Hit the tires in the group** {message.chat.title}\n\n» **unban Assistant first if you want to use this bot.**"
            )
            return
        ...
        if message.chat.username:
            try:
                ...
                await message.reply_text(
                    f"❌ **@{ubot.username} Assistant sleeping on someone's lap can't join try manually**\n\n**Reason**: `{e}`"
                )
                return
        else:
            try:
    ...
    if replied:
        if replied.video or replied.document:
            ...
            loser = await replied.reply("📥 **Downloading the vedio...**")
            ...
            if len(message.command) < 2:
                ...
                if pq == "720" or "480" or "360":
                    ...
                    Q = 720
                    await loser.edit(
                        "» **Only 720, 480, 360 which are allowed** \n💡 **Now Stream Video Inside 720p**"
                    )
            try:
                if replied.video:
...
""",
        ...
        if len(message.command) < 2:
            await message.reply(
                "» Reply To **file video** or **provide something to search for.**"
            )
        else:
            what = "Query Given"
            await LOG_CHAT(message, what)
            ...
            if search == 0:
                ...
                if veez == 0:
                    await loser.edit(f"❌ yt-dl problems detected\n\n» `{ytlink}`")
                else:
                    if chat_id in QUEUE:
                        ...
                        await app.send_message(
                                chat_id,
                                f"""
💡 **Streaming video started**

🏷 **Name:** [{songname[:999]}]({url})
⏱️ **Duration Time:** {duration}
🎧 **Requested By:** {requester}
...
async def playlist(client, m: Message):
    ...
    if chat_id in QUEUE:
        ...
        await m.reply("**❌ Didn't twist anything**")
